...
SYCL_EXTERNAL void row_cuda_bvp_set(double *u, int s, int r,
  ...
  double pi = 4.0 * (double)sycl::atan(1.0);
  double pip = 2.0 * (double)sycl::atan(1.0);
...
SYCL_EXTERNAL void row_cuda_bvp_1c2a(double *u, int s, int r,
  ...
  // 2a
  double sum = u[r * s + m - r], e = 0, y, tmp;
  for (k = s - 2; k >= 0; k--) {
    // u[m+k*r]+=u[m+(k+1)*r];
    tmp = sum;
    y = u[m + k * r] + e;
...
double row_bvp(int s, int r, int block_size, double *u_h) {
  ...
  */
  ...
  /*
  DPCT1012:162: Detected kernel execution time measurement pattern and generated
  ...
  */
  ...
  /*
  DPCT1012:163: Detected kernel execution time measurement pattern and generated
  an initial code for time measurements in SYCL. You can change the way time is
  measured depending on your goals.
  */
  stop->wait();
